...
- 🛠️ **手动部署** - 逐步部署，完全控制每个步骤
- ⚙️ **配置向导** - 引导式配置服务器、桩、VPN 参数
- 📋 **服务管理** - 启动、停止、重启、查看日志（实时/历史/按时间导出）等 systemd 服务操作
...
├── docs/
│   └── DEPLOYMENT_GUIDE.md  # 详细部署指南（原理、手动部署）
└── lib/
    ...
    ├── wireguard.sh         # WireGuard VPN 安装与配置
    ├── config.sh            # 配置管理（服务器、桩、VPN 配置、日志查看）
...
6. 部署 JAR 文件
7. 部署配置文件
8. 部署系统服务（自动同步 VPN IP 到 Nacos 注册地址）
9. 配置并启动服务
...
- 查看日志：
  - 实时日志（`journalctl -f`）
  - 最近日志（自定义行数）
  - 按时间范围导出日志（支持 `YYYY-MM-DD`、`-1h` 等格式）
  - 部署脚本日志
- 启用/禁用开机自启
...
如需了解部署原理（WireGuard VPN 组网、Nacos 注册机制、systemd 服务配置）或手动分步部署，请参阅 [部署指南](docs/DEPLOYMENT_GUIDE.md)。

## 📝 更新日志

### v2.5.0 (2026-03-29)
- 🐛 修复一键部署时 Nacos 注册 IP 未同步 VPN IP 的问题
- ✨ 新增日志查看菜单：实时日志、按行数查看、按时间范围导出
- 📄 添加详细部署指南文档
